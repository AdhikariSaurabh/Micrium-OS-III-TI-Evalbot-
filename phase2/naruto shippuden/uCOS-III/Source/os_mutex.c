...
TASK_TCB_TREE234 frtemp[20];
static int frtempl=0;
...
//////////////////////////////////////////////////////New mutex create with resource ceiling parameter
void  OSMutexCreate_Mod (OS_MUTEX    *p_mutex,
                     CPU_CHAR    *p_name,
                     OS_ERR      *p_err,
                     CPU_INT16U res_ceiling)
{
    CPU_SR_ALLOC();
...
///////////////////new mutex pend implementation//////////////////////////////////////////



void  OSMutexPend_Mod (OS_MUTEX   *p_mutex,
                   OS_TICK     timeout,
                   OS_OPT      opt,
                   CPU_TS     *p_ts,
                   OS_ERR     *p_err)
{
    ...
    if(((OSTCBCurPtr->MutexAcq > (CPU_INT08U)0)) || (system_ceiling==0) || p_mutex->OwnerTCBPtr==OSTCBCurPtr)  //the curr task is holding mutex(cs), then grant this mutex also to the the current task provided it's nesting ctr==0
    {                                                                                                                      //OR no task is holding any mutex i.e system_ceiling=0                      
         //grant the mutex acess
         p_mutex->OwnerTCBPtr       =  OSTCBCurPtr;          /* Yes, caller may proceed              */
         p_mutex->OwnerNestingCtr=p_mutex->OwnerNestingCtr+1;   //nesting of the same mutex with same task
         OSTCBCurPtr->MutexAcq=OSTCBCurPtr->MutexAcq+1; //increase acquired mutexes
         if (p_ts != (CPU_TS *)0) {
            *p_ts                   = p_mutex->TS;
          }
         //store this mutex in a AVL tree based on resource ceiling 
         mdl=mdl%20;
         md[mdl].res_ceil=p_mutex->res_ceil;
         md[mdl].p_mutex=p_mutex;
         
         AT=Ainsert(AT,&md[mdl]);    //inserting mutex into tree
         set_min_sys_ceil(AT);   //update system ceiling
         
         mdl++;
         
         //updating the system ceiling now 
          
         
         CPU_CRITICAL_EXIT();
         //no blocking or rescheduling takes place
         // I think it should return the function here
        return;
    }
   
   
    //define a variable known as system_ceiling
    
    else if(system_ceiling<=min_tcb->t_tcb->data)    //the current task has lower or equal priority as compared to system ceiling
    {
       p_tcb = p_mutex->OwnerTCBPtr; //contains tcb of the task which owns the mutex
       current_tcb=OSTCBCurPtr;  //pcb of current task
       
       
       
         //OSTCBCurPtr->period is same as min_tcb->t_tcb->data; //the current task to be blocked 
      //OSReadyList modifylist
      //remove from scheduler and block all the tasks (OSTCBCurPtr to  Blocking Task+1 priority) and insert in blocked task tree
      //remove the current task from ready list
       while (min_tcb->t_tcb->data != p_tcb->period) //case 1 :different period, tree
       {
         //insert into 234 tree
         frtempl=frtempl%20;
         frtemp[frtempl].period=min_tcb->t_tcb->period;
         frtemp[frtempl].data=min_tcb->t_tcb->data;
         frtemp[frtempl].p_tcb=min_tcb->t_tcb->p_tcb;
         fr_result=add234(tree234_blocked,&frtemp[frtempl]);
         frtempl++;
         
         //structure(234) <=> structure (splay)
         //remove from splay
         SH=delete_n(min_tcb->t_tcb,SH);
         set_min_ptr(SH);
           
       }
       
       if(min_tcb->next!=NULL)  // if the mutex owner taks exists as a linked list
       {
        while(min_tcb->next!=NULL)    //same period task
        {
          if(min_tcb->t_tcb->p_tcb== p_tcb)   //this task needs to be scheduled among the linked list
          {
            ttempl=ttempl%20;
            ttemp[ttempl].period=min_tcb->t_tcb->period;
            ttemp[ttempl].data=min_tcb->t_tcb->data;
            ttemp[ttempl].p_tcb=min_tcb->t_tcb->p_tcb;
            SH=delete_n(min_tcb->t_tcb,SH);
          }
         ...
         SH=insert(&ttemp[ttempl],SH);
         set_min_ptr(SH);
         ttempl++;
        
        }
       OS_RdyListRemove(current_tcb);
       if(SH!=NULL)
        {
             OS_PrioInsert(6);
      ...
      //ready list change, context swith from current task to mutex owner task present in the scheduler
       
       
       /*
       
       //OS_RdyListRemove(current_tcb);
       //not sure if OSTCBCurPtr= mutex owner task or scheduled automatically 
       //OS_PrioInsert(6); //insert dummy prio for mutex owner task
       //OS_RdyListInsertTail(p_tcb);  //insert this task in rdylist
       //OSSched();
       //insert 
       
      //block the current task ,place in blocked list
      //inherit the pritriority -> means remove from scheduler and place in blocked tree
      //no updation of system ceiling
    ...
    else
    {
      ...
      {
      //grant the resource to the task
      ...
      //store mutex value in a tree 
      //update system ceiling
      //no blocking or rescheduling takes place
         if(system_ceiling< current_mutex_resource_ceiling)
         {
           system_ceiling=current_mutex_resource_ceiling;
         }
    ...
    */
    else if(system_ceiling>min_tcb->t_tcb->data)  //task is at higher priority than system ceiling
    {
       //grant acess to resource provided the mutex is not held by any other task
      if(p_mutex->OwnerNestingCtr==0)    //mutex is free
      {
         p_mutex->OwnerTCBPtr       =  OSTCBCurPtr;          /* Yes, caller may proceed                                */
         OSTCBCurPtr->MutexAcq=OSTCBCurPtr->MutexAcq+1; //increase acquired mutexes
         if (p_ts != (CPU_TS *)0) {
...
///////////////////////////New mutex post implementation///////////////////////////////////
void  OSMutexPost_Mod (OS_MUTEX  *p_mutex,
                   OS_OPT     opt,
                   OS_ERR    *p_err)
{
    //OS_PEND_LIST  *p_pend_list;
    ...
    ts          = OS_TS_GET();                              /* Get timestamp                                          */
    p_mutex->TS = ts;
    if(p_mutex->OwnerTCBPtr==OSTCBCurPtr)
    {
     OSTCBCurPtr->MutexAcq=OSTCBCurPtr->MutexAcq-1;
     p_mutex->OwnerTCBPtr=(OS_TCB       *)0;;
     p_mutex->OwnerNestingCtr=p_mutex->OwnerNestingCtr-1;
     md[mdl].res_ceil=p_mutex->res_ceil;
     md[mdl].p_mutex=p_mutex;
     AT=AdeleteNode(AT,&md[mdl]);
     set_min_sys_ceil(AT);
     
     if(count234(tree234_blocked)!=0)
     {
      while(count234(tree234_blocked)!=0)   //blocked tasks are present in the tree
      {
       
        min_tcb_tree234=findrel234(tree234_blocked,NULL,mycmp, REL234_GT);
        
        ttempl=ttempl%20;
         ttemp[ttempl].period=min_tcb_tree234->period;
         ttemp[ttempl].data=min_tcb_tree234->data;
         ttemp[ttempl].p_tcb=min_tcb_tree234->p_tcb;
         SH=insert(&ttemp[ttempl],SH);
         ttempl++;
         
         del234(tree234_blocked,min_tcb_tree234);
     ...
     //remove from mutex from AVL tree
     //AT=AdeleteNode(AT,(p_mutex)
                    
     //   if(tree blocked!=NULL)
    // {
       //remove all the tasks from tree such that > system ceiling
