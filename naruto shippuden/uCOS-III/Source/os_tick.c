...
void OS_InsertReadyList(void)		
{		
    CPU_TS ts_start20 = 0;		
    CPU_TS ts_end20 = 0;		
    ts_start20 = OS_TS_GET();		
    OS_TCB *p_tcb;		
    int t=0;
  
    while((h.TASK[0])->data == OSTickCtr)	
  {		
    p_tcb = h.TASK[0]->p_tcb;		
  // p_tcb->prio 		
    CPU_STK_SIZE   i; 		
...
///////////////////////////////////////// 
     ...
     // setting the new instances of the task 
     NextPeriod[hcnt].period = h.TASK[0]->period;		
     NextPeriod[hcnt].data = (h.TASK[0]->period + OSTickCtr);		
     NextPeriod[hcnt].p_tcb = p_tcb;
     // removing the previous instance of the task
     heap_pop(&h);        
      
     
     
   // Inserting the next instance of the task in the heap  
     heap_push(&h,&NextPeriod[hcnt]);		
     
       
     
     j12=j12%100;
     temp_i[j12].period=0;
      temp_i[j12].data=NextPeriod[hcnt].period;
     //temp_ins.flag_in_rdylist=0;
     temp_i[j12].p_tcb=p_tcb;
     SH=insert(&temp_i[j12],SH);
     OS_RdyListRemove(min_tcb->p_tcb);
     set_min_ptr(SH);
     
     
     OS_PrioInsert(6);
     period_of_task_in_rdylist=min_tcb->data;
     min_tcb->p_tcb->Prio=6;
     OS_RdyListInsertTail(min_tcb->p_tcb);
     
     j12++;
     hcnt++;
     		
    ts_end20 = OS_TS_GET() - ts_start20;		
...
void  OS_TickTask (void *p_arg)
{
    ...
    while (DEF_ON) {
       ...
       // if (err == OS_ERR_NONE) {
		...
		OS_InsertReadyList();  // insertion of the tasks into the ready list
