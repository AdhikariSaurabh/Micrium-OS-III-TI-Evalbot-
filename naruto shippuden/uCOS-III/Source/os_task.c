...
TASK[h.count].data = data+OSTickCtr;
TASK[h.count].p_tcb = p_tcb;
heap_push(&h,&TASK[h.count]);

j11=j11%5; 

temp_ins[j11].period=0;
temp_ins[j11].data=data;
//temp_ins.flag_in_rdylist=0;
temp_ins[j11].p_tcb=p_tcb;
SH=insert(&temp_ins[j11],SH);
set_min_ptr(SH);
...
j11++;
  ...
  {		
   OSTaskCreate((OS_TCB     *) p_tcb, (CPU_CHAR   *)p_name, (OS_TASK_PTR ) p_task, (void       *) p_arg, (OS_PRIO     ) prio, (CPU_STK    *)p_stk_base, (CPU_STK_SIZE) stk_limit, (CPU_STK_SIZE) stk_size, (OS_MSG_QTY  ) q_size, (OS_TICK     ) time_quanta, (void       *)p_ext, (OS_OPT      )opt, (OS_ERR     *)p_err);		
  }		
}
