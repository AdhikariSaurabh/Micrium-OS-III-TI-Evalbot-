...
#include <os.h>
#include "tree.h"
extern struct heap h;            // defining the heap as the extern 
int cnt = 0;		
struct task Newperiod[100];
...
{		
 ...
 //   CPU_TS ts_end10 = OS_TS_GET() - ts_start;		
    //printf("Time elapsed2 = %d \n", (int)ts_end );		
#if OS_CFG_DBG_EN > 0u		
    ...
    Newperiod[cnt].period = h.TASK[0]->period;		
     Newperiod[cnt].data = (h.TASK[0]->period + OSTickCtr);		
     Newperiod[cnt].p_tcb = p_tcb;
   //Saurabh		
    OS_CRITICAL_ENTER();
    ts_start8 = OS_TS_GET();
    ...
    heap_pop(&h);
    ts_end9 = OS_TS_GET();
    ts_end3=ts_end9 - ts_start8;
    
    OS_CRITICAL_EXIT_NO_SCHED();
     heap_push(&h,&Newperiod[cnt]);		
     cnt++;		
  }    		
...
void  OS_TickTask (void *p_arg)
{
    ...
    while (DEF_ON) {
       ...
       // if (err == OS_ERR_NONE) {
		...
		Insert_Ready_List();  // insertion of the tasks into the ready list
